...
def main():
    ...
    # === Build confusion matrix (percentage-based) ===
    df_confusion_percent = pd.crosstab(
        pd.Series(y_true_labels, name='Actual'),
        pd.Series(y_pred_labels, name='Predicted'),
        normalize='index'  # Normalize per row (true class)
    ) * 100
    df_confusion_percent = df_confusion_percent.round(2)

    # === Save confusion matrix CSV ===
    cm_csv_path = f"./{args.path_out}_{args.dataset}_{args.weight}_confusion_matrix_percent.csv"
    df_confusion_percent.to_csv(cm_csv_path)
    print(f"✅ Confusion matrix (percentage) saved to: {cm_csv_path}")

    # === Save confusion matrix heatmap (improved for many classes) ===
    num_classes = len(df_confusion_percent)

    # Ukuran figure disesuaikan jumlah kelas
    ...
    sns.heatmap(
        ...
        cmap='Blues',
        cbar=True,
        annot_kws={"size": 6}  # ukuran teks angka kecil
    )
    plt.title(f"{args.dataset} – Confusion Matrix (Percent per Actual Class)")
    plt.ylabel("Actual")
    plt.xlabel("Predicted")

    # Rotasi label X agar tidak overlap
    plt.xticks(rotation=90, fontsize=6)
    plt.yticks(rotation=0, fontsize=6)

    plt.tight_layout()

    # Simpan sebagai PNG dan PDF
    cm_png_path = f"./{args.path_out}_{args.dataset}_{args.weight}_confusion_matrix_percent.png"
    cm_pdf_path = f"./{args.path_out}_{args.dataset}_{args.weight}_confusion_matrix_percent.pdf"

    plt.savefig(cm_png_path, dpi=300)
    plt.savefig(cm_pdf_path)
    plt.close()

    print(f"🖼️ Confusion matrix heatmap saved to: {cm_png_path} and {cm_pdf_path}")

    

    # === Save misclassified images from y_test ===
    misclassified_dir = f"./{args.path_out}_{args.dataset}_{args.weight}_misclassified"
    os.makedirs(misclassified_dir, exist_ok=True)

    for idx, (true_idx, pred_idx) in enumerate(zip(y_test_label, y_pred_class)):
        if true_idx != pred_idx:
            true_label = label_mapping[true_idx]
            pred_label = label_mapping[pred_idx]

            # Retrieve the image from y_test (assumed as numpy array)
            img_array = y_test[idx]
            try:
                ...
                filename = f"{true_label} - {pred_label}_{idx}.jpg"
